...
// --- CONFIGURATION ---
const PHP_API_BASE_URL = 'http://localhost:8000/saviskar_api'; // Local PHP built-in server
...
function updateHeaderInfo() {
    ...
    if (currentUser && greetingEl && tokenEl) {
        ...
        let label = "Token: ";
        let tokenValue = `SVSK${(currentUser.id || 0).toString().padStart(4, '0')}`;
    ...
    try {
        ...
        try {
            ...
            if (currentUser && !currentUser.id) {
                console.warn("Corrupted currentUser found (missing id), clearing.");
        ...
        if (currentUser) {
            try {
                ...
                if (mainContent) {
                     mainContent.innerHTML = `<div class="form-card" style="margin-top:50px;"><h2 class="form-title">Connection Error</h2><p>${apiError.message || 'We couldn\'t load your data from the server. Please refresh the page or try again later.'}</p></div>`;
                }
                updateHeaderInfo();
                updateNavLinksVisibility();
            }
        ...
        console.error("CRITICAL FATAL ERROR:", globalError);
        document.body.innerHTML = `<div style="padding: 50px; text-align: center; font-family: sans-serif; background: white; color: red;">
            <h1>Critical Frontend Crash</h1>
            <p>${globalError.toString()}</p>
            <p>${globalError.stack}</p>
            <button onclick="localStorage.clear(); location.reload();" style="padding: 10px 20px; font-size: 16px; cursor: pointer;">Clear Data & Reload</button>
        </div>`;
    }
});
